...
export class ItemService {
  ...
  updateItem(item: Item): Observable<Item>{
    return this.http.post<Item>(`${config.apiUrl}/item/update`, item, this.options)
    .pipe();
  }

  getFilterItems(item: Item): Observable<Array<Item>> {
    return this.http.get<Array<Item>>(`${config.apiUrl}/items/filter?filter=${encodeURI(JSON.stringify(item))}`);
  }
}
